...
def _tag_filter(features_dict, tags, which_tags=None):
    ...
    '''
    sample_tags = features_dict['tags']
    sample_tags = tf.expand_dims(sample_tags, axis=1)  # (m, 1) Tensor, where m is the number of tags in the dictionary
    sample_tags = tf.equal(sample_tags, tags)  # (m, n) Tensor, where n is the number of studied tags

    return tf.reduce_any(sample_tags) # returns True if and only if at least one feature tag is in the desired 'tags' list
...
def _tag_filter_hotenc_mask(features_dict, tags):
    ...
    sample_tags = tf.expand_dims(sample_tags, axis=1)  # (m, 1) Tensor, where m is the number of tags in the dictionary
    sample_tags = tf.equal(sample_tags, tags)  # (m, n) Tensor, where n is the number of studied tags
    sample_tags = tf.reduce_any(sample_tags, axis=0)  # (n,) boolean Tensor, with True in each position where a whitelist tag is present
    sample_tags = tf.cast(sample_tags, tf.int64)  # (n,) Tensor of 1’s and 0’s
    features_dict['tags']=sample_tags
...
def generate_datasets(tfrecords, audio_format, split=None, which_split=None, sample_rate=16000, num_mels=96, batch_size=32, block_length=1, cycle_length=1, shuffle=True, shuffle_buffer_size=10000, window_length=15, window_random=False, with_tids=None, with_tags=None, merge_tags=None, num_tags=155, num_tags_db=1, default_tags_db=None, default_tags_db_valid=None, repeat=None, as_tuple=True):
    ...
    if num_tags_db == 1:
        # add feature 'tags'
        AUDIO_FEATURES_DESCRIPTION['tags'] = tf.io.VarLenFeature(tf.string)

    else:
        default_tags_db = default_tags_db or 0
        # add feature 'tags_i' for each i-th tags database provided
        for i in range(num_tags_db):
            AUDIO_FEATURES_DESCRIPTION['tags_' + str(i)] = tf.io.VarLenFeature( tf.string)
    ...
    for files_list in tfrecords_split:
        ...
        if with_tags is not None:
            ...
            dataset = dataset.filter(lambda x: _tag_filter(x, with_tags, default_tags_db))
            dataset = dataset.map(lambda y: _tag_filter_hotenc_mask(y, tags=with_tags))
...
def generate_datasets_from_dir(tfrecords_dir, audio_format, split=None, which_split=None, sample_rate=16000, num_mels=96, batch_size=32, block_length=1, cycle_length=1, shuffle=True, shuffle_buffer_size=10000, window_length=15, window_random=False, with_tids=None, with_tags=None, merge_tags=None, num_tags=155, num_tags_db=1, default_tags_db=None, default_tags_db_valid=None, repeat=1, as_tuple=True):
    ...
    for file in os.listdir(os.path.expanduser(tfrecords_dir)):
        ...
        if file.endswith(".tfrecord") and file.split('_')[0] == audio_format:
            tfrecords.append(os.path.abspath(os.path.join(tfrecords_dir, file))) #The abspath ensures that the string is in path format
