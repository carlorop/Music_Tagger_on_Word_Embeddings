...
class Matrix():
    ...
    def matrix(self, lastfm, tags=None, dim=3, save_to=None):
        ...
        # initialize matrix
        matrix = sparse.DOK((len(tags),) * dim,
                            dtype=np.int32)  # sparse dict-of-keys matrix (for easy creation, awful for calculations)
        ...
        def count_intersect_tags(tags):
            ...
            tids = set(
                tids_list.pop())  # start with shortest list of tids to improve performance; convert to set to be able to intersect
        ...
        def count_intersect_tags_recursive(tags_idxs,
                                           dim):  # recursively iterate count_intersect_tags dim times; avoid repetitions such as 'rock AND pop AND folk' vs. 'rock AND folk AND pop' vs. 'folk AND pop AND rock'
            if dim >= 1:
    ...
    def tags_and(self, tags):
        ...
        if tags.dtype == int:
            ...
            idxs = np.where(np.array([self.m_tags] * len(tags)) == tags[:, None])[
                1]  # 'numpy version' of idxs = [self.m_tags.index(tag) for tag in tags]
            assert len(idxs) == len(
                tags), 'some of the tags you inserted might not exist in the matrix'  # sanity check (np.where will not return errors if correspondent idx does not exist)
    ...
    def with_many_without_one(self, with_tags,
                              without_tags):  # when with_one_without_one is needed, this function should be preferred
    ...
    def are_equivalent(self, threshold=0.8, verbose=False):
        ...
        if verbose:
            ...
            for x, y in zip(*np.where(np.tril(matrix > 0))):
                count += 1
                print('{0:>3}. {1:3.1f}% of {3} is {4}\n{5:>3}  {2:3.1f}% of {4} is {3}\n'.format(count, correlation[
                    x, y] * 100, correlation[y, x] * 100, self.m_tags[x], self.m_tags[y], ' ' * len(str(count))))
        return matrix
        ...
        if verbose:
            ...
            for x, y in zip(*np.where(matrix > 0)):
                ...
                print('{0:>3}. {1:3.1f}% of {3} is {4}\n{5}(but {2:3.1f}% of {4} is {3})\n'.format(count, correlation[
                    x, y] * 100, correlation[y, x] * 100, self.m_tags[x], self.m_tags[y], ' ' * max(0, len(str(
                    count)) - 3)))
        return matrix
    ...
    def all_tag_is_either(self, threshold=0.7, verbose=False):
        ...
        if verbose:
            ...
            for x, y, z in zip(*np.where(matrix > 0)):
                ...
                count += 1
                print('{0:>3}. {1:3.1f}% of {2} is either {3}\n{5}or {4}\n'.format(count, correlation[x, y, z] * 100,
                                                                                   self.m_tags[x], self.m_tags[y],
                                                                                   self.m_tags[z],
                                                                                   ' ' * (22 + len(self.m_tags[x]))))
        return matrix


def crazysum(n, s, k):
    return int(
        (math.factorial(n + k - 1) / (math.factorial(n - 1) * math.factorial(k + 1))) * ((n - 1) * s + k + 3 - 2 * n))
